...
const History = ({ historyItems, onSelectItem, onClearHistory }: HistoryProps) => {
  return (
    <Sidebar variant="inset" className="border-r border-border/40 bg-secondary">
      <SidebarHeader className="flex flex-row items-center justify-between p-4 border-b border-border/40">
        ...
        {historyItems.length > 0 && (
          <Button 
            ...
            className="text-foreground hover:text-destructive h-7 px-2"
          >
      ...
      <SidebarContent className="p-0 bg-secondary">
        <ScrollArea className="h-[calc(100vh-5rem)]">
          {historyItems.length > 0 ? (
              ...
              {historyItems.map((item) => (
                <SidebarMenuItem key={item.id} className="px-4">
                  <SidebarMenuButton
                    ...
                    className={cn(
                      "flex w-full items-center justify-center gap-3 rounded-md px-4 py-3 hover:bg-accent text-sm transition-colors",
                      "text-center font-normal text-foreground"
                    )}
                  >
            ...
            <div className="flex flex-col items-center justify-center h-40 px-4 py-8 text-center">
              <Clock className="h-10 w-10 text-foreground/40 mb-4" />
              <p className="text-sm text-foreground">No history yet</p>
              <p className="mt-1 text-xs text-foreground/70">Summarized texts will appear here</p>
            </div>
          )}
        </ScrollArea>
      </SidebarContent>
